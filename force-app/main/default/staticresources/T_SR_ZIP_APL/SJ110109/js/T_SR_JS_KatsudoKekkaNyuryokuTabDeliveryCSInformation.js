...
function reloadDeliveryInformationBefore() {
    try {
        var prms = [];
        var valNoCyumon = $('[id$=hidCyumonNo]').val();
        var valNoCyumonEd = $('[id$=hidCyumonedNo]').val();
        $('.S10101-check-annai').each(function(idx, elm) {
          elm = $(elm);
          if (elm.prop('checked')) {
            prms.push({
              noCyumon: valNoCyumon,
              noCyumonEd: valNoCyumonEd,
              cdAnnai: elm.val()
            });
          }
        });
        window.parent.parent.updateTipGrayCSI_SG10040(prms);
    } catch(e) {
    }
}
