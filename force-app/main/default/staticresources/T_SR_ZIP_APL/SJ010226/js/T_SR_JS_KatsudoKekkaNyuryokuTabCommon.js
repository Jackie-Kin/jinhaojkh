...
(function () {
    // 改善対応46 2019/04/09 ADD Start
    try {
        if (window.parent.parent.location.href.indexOf('T_P_KanbanPost') >= 0) {
    ...
    S10101InputActivityResult = {
        ...
        ctrlEntryBtn: function () {
            ...
            if (checkedChb.length > 0 || $('[id$=S10101-iar-status]').val() != '') {
                ...
                $('#S10101-iar-btn-entry').addClass('no-select');
            }
        },
        ctrlNextPlan: function (initFlg) {
            ...
            if (endChb.length > 0) {
                // 非活性化されていない場合のみ処理を行う
                if ($('.S10101-iar-time > select').is(':disabled') === false) {
                    // 活動予定日非活性化処理
                    // 初期表示の場合、値をクリアしない
                    if (initFlg !== '1') {
                        $('[id$=startDate]').val('');
                    ...
                    // 初期表示の場合、値をクリアしない
                    if (initFlg !== '1') {
                        $('.S10101-iar-time > select').val('');
                    }
                    $('.S10101-iar-time > select').prop('disabled', true);
                    // 次回接触方法 非活性化処理
                ...
                // 開始時刻、終了時刻 活性化処理
                $('.S10101-iar-time > select').prop('disabled', false);
                // 次回接触方法 活性化処理
        ...
        commit: function () {
            ...
            // 登録ボタン押下処理
            if ($('#S10101-iar-btn-entry').hasClass('no-select')) {
                return;
            }
            ...
            var activityPlanDate = $('[id$=startDate]').val();
            var startTime = $('#S10101-iar-start-hour').val() + $('#S10101-iar-start-minutes').val();
            var endTime = $('#S10101-iar-end-hour').val() + $('#S10101-iar-end-minutes').val();
            ...
            if ($('#important-memo').val() != $('#important-memo-syoki').val()
                    || $('[id$=S10101-iar-preZymemoChangeFlg]').val() === '1'
                    || $('[id$=S10101-iar-preZymemoChangeFlg]').val() === '2'
            ) {
            ...
            var data = {
                ...
                inputActivityResult: {
                    ...
                    katsudoKekkaEx: $('[id$=S10101-iar-katsudoKekkaEx]').val(),
                    preKbCall: $('[id$=S10101-iar-preKbCall]').val()
                },
                input: {
                    contactId: $('[id$=S10101-iar-contactId]').val(),
                    sharyoId: $('[id$=S10101-iar-sharyoId]').val(),
        ...
        checkInput: function () {
            ...
            var endChb = $('#S10101-iar-activity-result .S10101-iar-chk-btn:checked[end = "1"]');
            if (endChb.length > 0 && $('.S10101-iar-time > select').is(':disabled') === false) {
            ...
            var satei = $('#S10101-iar-activity-result .S10101-iar-chk-btn:checked[value = "' + S10101Const.SATEI_BTN + '"]');
            if (satei.length > 0) {
                var sateiNenshiki = $('#S10101-iar-activity-ddSateiSyaNe').val();
                if (sateiNenshiki !== '' && (sateiNenshiki < 1868 || sateiNenshiki > 2117)) {
                    return S10101InputActivityResult.MSG_ERR_SATEINENSHIKI_RANGE;
                }
            }
        ...
        MSG_ERR_SATEINENSHIKI_RANGE: S10101Const.MSG_0019/*'年式は1868～2117の期間内で入力してください。'*/,
    },
    // ボタン
    S10101Buttons = {
        // ボタン初期化
