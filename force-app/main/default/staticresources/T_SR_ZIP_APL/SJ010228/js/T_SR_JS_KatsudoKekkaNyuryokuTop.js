...
(function (base) {
    ...
    ns.Selectors = {
         ...
         * 隠し項目：名前空間
         */
        HIDDEN_NAME_SPACE_VALUE: "#S00301-hidden-name-space"
    };
    ...
    function onClose(options, container) {
        ...
        if (window.parent && !options.ProgramId && ($('[id$=S00301-hidden-contactId]').val() != '')) {
            spmMasterLayout.LoadStart();
            // ×ボタンの場合チップID取得する
            ns._options = options;
            s00301GetChipId();
        }
        else if (window.parent) {
            var programId = $(ns.Selectors.HIDDEN_NAME_SPACE_VALUE).val() + 'T_P_KatsudoKekkaNyuryokuTop';
            ...
            if (options.ProgramId) {
                sendOptions.ProgramId = programId;
            }
